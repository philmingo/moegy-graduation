...
export default function ScannerPageWrapper() {
  ...
  const handleScan = useCallback(
    (data: string | any) => {
      ...
      setScanning(false)
      // REMOVED: setScannerKey increment - this was causing unnecessary remounts
      // The scanner will naturally be ready for the next scan without remounting
    },
  ...
  const handleScannerRefresh = useCallback(() => {
    console.log("Manual scanner refresh triggered")
    setScannerKey((prev) => prev + 1)
  }, [])

  // Stop scanning function - simplified with reliance on child component cleanup
  const handleStopScanning = useCallback(() => {
    console.log("🛑 [SCANNER-WRAPPER] === STOP SCANNING REQUESTED ===", {
      timestamp: new Date().toISOString(),
      currentlyScanning: scanning
    })
    
    // Simply update the scanning state - the QrScanner component will handle camera cleanup
    setScanning(false)
    console.log("🛑 [SCANNER-WRAPPER] Set scanning state to false")
    
    // Trigger scanner remount to ensure clean state
    console.log("🛑 [SCANNER-WRAPPER] Triggering scanner remount")
    setScannerKey((prev) => prev + 1)
    
    console.log("🛑 [SCANNER-WRAPPER] === STOP SCANNING COMPLETED ===")
  }, [scanning])

  // Start scanning function
  const handleStartScanning = useCallback(() => {
    console.log("▶️ [SCANNER-WRAPPER] === START SCANNING REQUESTED ===", {
      timestamp: new Date().toISOString(),
      currentlyScanning: scanning
    })
    setScanning(true)
    console.log("▶️ [SCANNER-WRAPPER] Set scanning state to true")
  }, [scanning])
